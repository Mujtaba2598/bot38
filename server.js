...
// ==================== API KEY MANAGEMENT ====================
app.post('/api/set-keys', authenticate, (req, res) => {
...
app.post('/api/set-api-keys', authenticate, async (req, res) => {
    ...
    if (!verification.success) return res.status(401).json({ success: false, message: 'Invalid Binance API keys. Enable Spot & Margin Trading.' });
    const users = readUsers();
    users[req.user.email].apiKey = encrypt(apiKey);
    users[req.user.email].secretKey = encrypt(secretKey);
    writeUsers(users);
    res.json({ success: true, message: 'API keys verified and saved!' });
});
...
app.get('/api/admin/users', authenticate, (req, res) => {
    ...
    const userList = Object.keys(users).map(email => ({
        email, hasApiKeys: !!users[email].apiKey, createdAt: users[email].createdAt, isOwner: users[email].isOwner
    }));
